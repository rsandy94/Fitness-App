...
## Overview
Personal fitness tracking PWA built with Expo, targeting iOS Safari (add to home screen). MVP focuses on Push workout day with full set tracking, weight logging, and progress graph. Future iterations add Pull/Lower/Upper days and APK export.

## Design Reference
**All screens must match the screenshots in `./Screenshots/` as closely as possible.** The screenshots are the source of truth for layout, colors, typography, icons, and interactions. Where the screenshots and this plan conflict, the screenshots take priority.

Screenshots folder: `D:\Personal Projects\Fitness App\Screenshots\`
- `Dashboard.PNG` — Home screen with weight log checklist
- `Weekly schedule.PNG` — Weekly workout schedule view
- `Workouts.PNG` — Exercise list for a workout day
- `Workout instructions.PNG` — Exercise detail with YouTube + instructions
- `Execerise Entry.PNG` — Active workout session with set tracking
- `Workout inventory.png` — Exercise data reference (Google Sheet)

## Tech Stack
| Layer | Choice | Version |
|-------|--------|---------|
| Framework | Expo + Expo Router | SDK 53 |
...
| Auth | Local PIN (AsyncStorage) | v1 only |

## Supabase Configuration
- **Project URL:** https://ubgmeqijydyzehxzkibj.supabase.co
- **Anon Key:** sb_publishable_GZlgEqbZKKHFOAlFexfHYA_PWHYJH9G
- **Storage Bucket:** Thumbnails (public)
- **Auth:** Single user, local PIN (no Supabase auth for v1)
...
-- RLS: Enable on all tables (v1: allow all for single user)
...
-- Seed: Weekly schedule (Sun=Push, Mon=Pull, Tue=Lower 1, Wed=Rest, Thu=Upper, Fri=Lower 2, Sat=Rest)
-- Note: Pull, Lower, Upper templates not created yet - will be null for v1
...
```sql
-- Policy: allow public read on Thumbnails bucket
create policy "public read thumbnails" on storage.objects for select using (bucket_id = 'Thumbnails');

-- Policy: allow upload to Thumbnails bucket
create policy "allow upload thumbnails" on storage.objects for insert with check (bucket_id = 'Thumbnails');
```
...
│   │   ├── index.tsx            # Dashboard (weight log + checklist)
│   │   ├── schedule.tsx         # Weekly workout schedule
│   │   └── progress.tsx         # Weight graph
│   ├── workout/
│   │   ── [templateId].tsx     # Exercise list for a workout
│   ├── exercise/
│   │   └── [exerciseId].tsx     # YouTube + instructions
│   └── session/
│       └── [sessionId].tsx      # Active workout tracking
├── components/
│   ├── ExerciseCard.tsx
│   ├── SetTracker.tsx
│   ├── WeightChart.tsx
...
├── assets/
│   └── thumbnails/              # Local thumbnails (for upload script)
├── scripts/
│   └── upload-thumbnails.ts     # One-time upload script
└── app.json
```
...
- [ ] Initialize Expo project with Expo Router + NativeWind
- [ ] Configure Supabase client with provided credentials
- [ ] Set up project structure (folders, base components)
- [ ] Create upload script for thumbnails

### Phase 2: Database & Data Layer
- [ ] Execute SQL scripts in Supabase
- [ ] Run thumbnail upload script
- [ ] Update exercise records with thumbnail URLs
- [ ] Create TanStack Query hooks for all tables
- [ ] Create Zustand store for active workout session
...
### Phase 4: Dashboard
- [ ] Weight log form (date + weight input)
- [ ] Today's checklist (weight check-in status)
- [ ] Weekly schedule summary

### Phase 5: Schedule Screen
- [ ] Weekly view with day cards
- [ ] Status indicators (scheduled/completed/missed)
- [ ] Navigation to workout detail

### Phase 6: Workout Detail
- [ ] Exercise list with thumbnails, sets, reps
- [ ] Target muscles display
- [ ] Navigation to exercise detail + start workout

### Phase 7: Exercise Detail
- [ ] YouTube video embed
- [ ] Muscle groups with primary/secondary labels
- [ ] Exercise instructions text

### Phase 8: Active Workout Session
- [ ] Set tracking table (set number, weight, reps)
- [ ] Add/remove sets dynamically
- [ ] Previous session data auto-population
- [ ] Finish workout → update status

### Phase 9: Progress Screen
- [ ] Weight graph (line chart over time)
- [ ] Date range selector
...
- **Auth:** Local PIN for v1 (single user)
- **Theme:** Dark mode only
- **Units:** lbs
- **Package Manager:** npm
- **Storage:** Supabase Storage bucket "Thumbnails" (public)
- **Schedule:** Sun=Push, Mon=Pull, Tue=Lower 1 (Quad), Wed=Rest, Thu=Upper, Fri=Lower 2 (Glute), Sat=Rest
- **MVP Scope:** Push workout only, add Pull/Lower/Upper in later iterations
